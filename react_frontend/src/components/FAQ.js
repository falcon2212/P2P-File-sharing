...
// import {Col, Container, Form, FormFile, FormGroup, ListGroup, Row} from "react-bootstrap";
...
//                         <Col className={"justify-content-end"}><ArrowUpSquareFill value={this.state.memberList[i]} onClick={(ev) => this.sendFile(ev)}/></Col>
//                     </Row>
//                 </ListGroup.Item>
//             );
//         }
...
class FAQ extends Component {
    ...
    createUserList() {
        ...
        for(var i=0; i<this.state.memberList.length; i++){
            html.push(
                <ListGroup.Item>
                    {this.state.memberList[i]}<Button value={this.state.memberList[i]} onClick={(ev) => this.sendFile(ev)}/>
                    {/*<Row className={"p-2"}>*/}
                    {/*    <Col>{this.state.memberList[i]}</Col>*/}
                    {/*    /!*<Col><Form.File onChange={(e) => {this.addFile(e)}} type={"file"} className={"custom-file-label"} label={this.state.memberList[i]} custom/></Col>*!/*/}
